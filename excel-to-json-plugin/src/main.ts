...
exports.methods = {
    ...
    'set-output-path': function (path: string) {
        console.log('[Excel Plugin] Setting output path:', path);
    },

    'convert-files': function (data: { files: string[], outputPath: string, generateTypes: boolean }) {
    ...
    'open-output-folder': function (outputPath: string) {
        ...
        try {
            ...
            if (process.platform === 'win32') {
                ...
                exec(`explorer "${resolvedPath}"`, { timeout: 5000 }, (error: any) => {
...
function convertExcelFiles(files: string[], outputPath: string, generateTypes: boolean) {
    files.forEach(file => {
        try {
...
function generateTypeDefinition(jsonData: any[], outputPath: string, baseName: string, sheetName: string, hasMultipleSheets: boolean) {
    try {
...
function generateValidPropertyName(key: string): string {
    // 如果包含中文或特殊字符，使用引号包围
    if (/[^\w$]/.test(key)) {
        return `"${key}"`;
    }
 ...
 * 过滤配置行，只保留实际数据行
 */
function processExcelData(rawData: any[]): Record<string, any> {
    ...
    // 找到KEY字段名
    const keyFieldName = Object.keys(headerRow).find(key =>
        key.includes('KEY') || key.includes('key') || key.includes('Key') ||
    ...
    const dataRows = rawData.filter((row, index) => {
        if (index < 4) return false; // 跳过前4行（字段映射、类型、server、client）
    ...
    dataRows.forEach(row => {
        ...
        Object.keys(fieldMapping).forEach(originalField => {
            ...
            // 跳过KEY字段，因为它已经作为对象的键
            if (originalField === keyFieldName) return;

            // 跳过空的映射字段
            if (!mappedField || mappedField === '') return;
 ...
 */
function generateTypeDefinitionForObject(data: Record<string, any>, outputPath: string, baseName: string, sheetName: string, hasMultipleSheets: boolean) {
    try {
